...
OUT_DIR = PROJECT_ROOT / "unified_dataset_optimal"
...
def process_single_image(args):
    ...
    # Process image (convert PBM to JPG, else fast copy)
    if img_path.suffix.lower() == ".pbm":
        dest_img = OUT_DIR / split / "images" / (new_stem + ".jpg")
        with Image.open(img_path) as im:
...
def process_datasets():
    ...
    for cls, records in dataset_records.items():
        print(f"Category '{cls}': {len(records)} images")
    ...
    tasks = []
    for img_path, labels in all_records:
    ...
    print(f"Starting parallel processing for {len(tasks)} images... This will be much faster.")

    total_images = len(tasks)
    total_positives = 0
    total_backgrounds = 0

    # Use ThreadPoolExecutor for fast I/O bound operations
...
"""
    ...
    print(f"Done! OPTIMAL Dataset Built:")
    print(f" - Total Images: {total_images}")
    print(f" - Labeled Images (Contain Objects): {total_positives}")
    print(f" - Background Images (Hard Negatives): {total_backgrounds}")
